...
@ApiTags('group API') // Swagger 태그 지정
@Controller('group')
export class GroupController {
    ...
    @ApiResponse({ status: 201, description: 'The Group record has been successfully created.', type: GroupDto })
    ...
    @ApiResponse({ status: 200, description: 'found *List로 반환합니다*', type: GroupDto, isArray: true })
    ...
    @ApiBody({ type: SearchGroupDto })
    @ApiResponse({ status: 302, description: 'success *List로 반환합니다*', type: MemberDto, isArray: true })
    @Post('ckeckGroup')
    ...
    @ApiBody({ type: SearchGroupDto })
    @ApiResponse({ status: 200, description: 'success (지워진 그룹 반환)', type: GroupDto })
    ...
    async deleteGroup(@Body() body: SearchGroupDto, @Res() res) {
        try {
            ...
            res.status(HttpStatus.OK).json({ message: "successfully deleted", ans });
        } catch (error) {
    ...
    @ApiBearerAuth() // Swagger에 Bearer Auth 정보 표시
    @ApiOperation({ summary: '리더 강요 (이거 쓰지 마세요, 테스트할 때만 쓰세요!!)', description: '리더를 강제로 설정합니다.' })
    @ApiBody({ type: SearchGroupDto })
    ...
    @ApiBody({ type: SearchGroupDto })
    @ApiResponse({ status: 200, description: 'Generated Code', type: GroupDto })
    @Post('makeInvitation')
